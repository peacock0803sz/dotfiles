...
- spec-kit関連ファイル (`.specify` や `specs/`)はGlobal gitignoreで除外済みなので、コミットしない
    - ただし、リポジトリの.gitignoreで上書きしている場合は従うこと
- `git add` と `git commit` をPermissionで許可しているので、それぞれ別コマンドとして実行すること (`&&` などで繋げない)
