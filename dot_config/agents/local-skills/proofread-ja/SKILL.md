...
AIが書いた日本語の下書きを、読み手が「人が書いた」と感じる自然な日本語に書き直す。

## 実行手順

1. 引数で指定されたファイルをReadツールで読み込む
2. 下記の校正ルールに従い、文章全体を書き直す
3. 校正後の文章で元ファイルをWriteツールで上書きする
4. 変更箇所の要約を簡潔に報告する
...
- 半角英数字の前後にスペースは入れない(日本語と英数字の間に余白を足さない)。
...
- Markdown内では hard wrap しない(1段落=1行)。指定文字数で改行を入れず、改行はソフトラップに任せる。
